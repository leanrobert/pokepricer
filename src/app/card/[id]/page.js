...
const PokemonCardDetails = ({ card }) => {
  return(
    <div className={`max-w-5xl self-center mb-8 border-blue-900 border-[1px] bg-gradient-to-br ${colors[card.types[0]].textColor} ${card.types.length === 1 ? `${colors[card.types[0]].bgFrom} ${colors[card.types[0]].bgTo}` : `${colors[card.types[0]].bgFrom} ${colors[card.types[1]].bgTo}`} text-zinc-800 rounded-lg p-3`}>
      <h2 className="font-bold text-center mb-3 text-black">Card Details</h2>
      <div className="flex justify-center gap-4">
        <div className="w-1/3 flex flex-col justify-center bg-sky-50 rounded-lg px-4 py-2">
        ...
        <div className="border-[1px] bg-sky-50 px-4 py-2 rounded-lg">
...
const CardPrices = async ({ card }) => {
  ...
  return (
    <div className={`flex flex-col justify-center w-2/3 border-blue-900 border-[1px] bg-gradient-to-br ${colors[card.types[0]].textColor} ${card.types.length === 1 ? `${colors[card.types[0]].bgFrom} ${colors[card.types[0]].bgTo}` : `${colors[card.types[0]].bgFrom} ${colors[card.types[1]].bgTo}`} text-zinc-800 rounded-lg p-4`}>
      <h2 className="font-bold text-lg text-center mb-3 text-black">Price Details</h2>
      <div className="bg-sky-50 rounded-xl py-2 px-4 flex justify-center">
