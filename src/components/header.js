...
const Header = () => {
  ...
  return (
    <>
      ...
      <nav className='sticky h-14 inset-x-0 top-0 z-30 w-full border-b border-white bg-white/75 backdrop-blur-lg transition-all'>
        <MaxWidthWrapper>
          <div className='flex h-14 items-center justify-between border-b border-white'>
            <Link href='/' className='flex items-center gap-2 z-40 font-semibold'>
              <Image src='/logo.png' alt='logo' width={64} height={64} className='bg-white rounded-full h-4 w-4' />
              Pokeprecios
            </Link>
            ...
            <div className='hidden sm:flex items-center space-x-4'>
              {!user ? (
                <>
                  <Link
                    href='/pricing'
                    className='inline-flex items-center justify-center rounded-md text-sm font-medium ring-offset-sky-50 hover:text-blue-900 hover:bg-sky-50'>
                    Pricing
                  </Link>
                  <Link href='/signin'>
                    <Button>
