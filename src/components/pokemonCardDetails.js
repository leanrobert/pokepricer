export const PokemonCardDetails = ({ card }) => {
  return(
    <div className={`lg:w-1/3 mx-4 max-w-5xl border-blue-900 border-[1px] bg-gradient-to-br text-zinc-800 rounded-lg p-3`}>
      ...
      <div className="flex flex-wrap lg:flex-nowrap justify-center gap-4">
        <div className="w-full lg:w-1/3 flex flex-col justify-center bg-sky-50 rounded-lg px-4 py-2">
          <p className="font-bold text-sm mb-3">Card Number: <span className="font-normal">{Number(card.number).toLocaleString('en-US', {minimumIntegerDigits: 3, useGrouping: false})}/{card.cardset.total}</span></p>
          <p className="font-bold text-sm mb-3">Card Rariry: <span className="font-normal">{card.rarity}</span></p>
          <p className="font-bold text-sm mb-3">Card Type: <span className="font-normal">{  card.types}</span></p>
          <p className="font-bold text-sm mb-3">Card HP: <span className="font-normal">{card.hp}</span></p>
          <p className="font-bold text-sm mb-3">Card Stage: <span className="font-normal">{card.subtypes}</span></p>
        </div>
        <div className="border-[1px] bg-sky-50 px-4 py-2 rounded-lg lg:w-2/3">
          ...
          {card.cardattacks?.map((attack, index) => (
            attack.abilities_name ? (
              <div className="px-1 w-full" key={index}>
              <div className="rounded-lg px-4 py-2 mb-1 border-blue-900 border-[1px] bg-sky-50">
              <p className="bg-red-600 text-sky-50 rounded-full mb-2 px-3 inline-block text-sm">{attack.abilities_name}</p>
              <p className="text-xs">{attack.abilities_text}</p>
              </div>
            </div>
            ) : (
            <div className="p-1 w-full" key={index}>
              <div className=" border-[1px] bg-sky-100 px-4 py-2 rounded-lg">
                <div className="flex justify-between">
                  <p className="text-sm font-semibold">{attack.attacks_name}</p>
                  <p className="font-semibold">{attack.attacks_damage}</p>
                </div>
                <p className="text-xs">{attack.attacks_text}</p>
              </div>
            </div>
          )))}
        </div>
